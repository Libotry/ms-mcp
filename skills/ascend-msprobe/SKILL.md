...
- msProbe：精度调试（dump 数据采集、精度比对、精度预检）
- msMemScope：内存调试（内存泄漏、内存踩踏、低效内存）

两者互补，共同构成昇腾调优工具链。

**Q: dump 数据量太大怎么办？**
- 使用 `statistics` 模式代替 `tensor` 模式
- 配置 `scope` 或 `list` 缩小采集范围
- 配置 `step` 只采集特定 step

**Q: 性能损耗大怎么办？**
- 使用 `async_dump: true` 异步采集
- Monitor 监测优先使用权重梯度（性能损耗 <1%）
- 避免长期开启 `tensor` 模式
...
- 可能是工具中的 item 操作引入同步导致
- 参考 FAQ 中的"模型计算结果改变原因分析"
