...
1. **所有 profiling / tuning 结论都必须来自真实华为 Ascend NPU 执行。** 只有在目标 workload 已经在真实 Ascend NPU 上跑完，并成功采集到 profiling artifact（如 `msprof` 导出、`torch-npu` profiler 结果、trace / csv / db / json）后，才允许进入分析、对比、收敛判断。
2. **严禁伪造数据。** 禁止使用 synthetic、mock、estimate、手工编造、复制粘贴或“为了演示流程”生成的 `metrics.json`、CSV、JSON、trace、before/after 数字、收敛结果。
3. **没有 Ascend 真机环境就必须停止。** 如果没有可用的华为 Ascend NPU、目标程序无法在 NPU 上运行、profiling 命令执行失败、或未成功产出 profiling artifact，必须明确告知用户当前阻塞，停止在 profiling/tuning 结果层面的后续动作。
4. **禁止无证据输出。** 在没有真实 profiling artifact 之前，不得输出“瓶颈已确认”“优化前后提升了 X%”“已收敛”“目标达成”等结论。
...
- 当前环境有可用的华为 Ascend NPU
- 目标 workload 能在 Ascend NPU 上真实运行
- profiling 工具可用，并会产出真实 artifact

如果以上任一条件不满足：**立即停止**，向用户说明缺失项；**不得**手工创建或补写任何 profiling 数据文件。
*(注：`run_profile.sh` 脚本内部会自动调用 `check_real_profiling_env.py` 执行硬核校验，如果没有 NPU 工具如 `npu-smi` 会直接报错拦截)*
...
仅允许分析来自上一步真实 Ascend NPU 执行产出的 profiling artifact。若数据来源不明、无法追溯到真实执行、或文件是手工创建的，占位的，必须停止并说明数据无效。
...
必须重新在真实华为 Ascend NPU 上运行修改后的 workload，并采集新一轮 profiling artifact。禁止复用旧数据、手工改写指标、或伪造“优化后”结果。
...
只有同时具备“优化前真实 profiling artifact”和“优化后真实 profiling artifact”时，才允许执行收敛判断。否则必须停止，并明确说明缺少真实对比数据。
